...
| **OBS-08** | ~~Media~~ → **corregida (2026-08-12)** | **Ya se pueden dar de baja presupuestos.** El listado ofrece "Eliminar" con confirmación ("Se dará de baja… No se borra físicamente, queda inactivo"). Verificado: al eliminar PR-0627, el total pasó de 50 a 49. Es baja lógica, como en Proveedores. |
...
| **OBS-17** | Baja / a investigar | **Un presupuesto Aceptado no aparece en Ventas.** La automatización Presupuesto→Venta funciona (Ventas = presupuestos Aceptados, verificado), pero PR-0584 (FERNANDEZ LORENA, activo, monto 130.000) quedó afuera sin diferencia de datos evidente. Ver [trazabilidad-roadmap](reportes/2026-08-10-trazabilidad-roadmap.md). |
| **OBS-16** | Baja | **El campo Teléfono acepta letras y las guarda.** En el alta de Proveedores, Teléfono es texto plano (sin tipo `tel` ni validación de formato): se cargó `ABCdef-letras!` y persistió tal cual (`PROV-0024`). Es calidad de datos, no integridad. El mismo campo existe en Clientes (calcado), así que heredará el problema al aplicarse la migración. |
